...
const ContactPage = () => {
	...
	const required = ['customer_name', 'email', 'phone', 'agreed', 'message', 'city'];
	...
	const formSubmitHandler = () => {
		...
		for (const key in formData) {
			...
			if (key !== 'note') {
				if (item === '' || item == null || (required.includes(key) && (item === '' || item == null))) {
					err.push(key);
				}
			}
		}
		...
		if (err.length > 0) {
			...
			if (!id && !name) {
				api.post('/contact/contact_page', ret)
					.then((response) => {
				...
				api.post('contact/product_request', {
					...ret,
					id_product: +id,
					product_name: name,
				})
					.then((response) => {
						openAlertBox('Uspešno ste poslali zahtev za proveru proizvoda na stanju.', 'success');
						setShowMessage(true);
						setFormData({
							customer_name: '',
							email: '',
