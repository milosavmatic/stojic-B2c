...
const CheckoutPage = ({ paymentoptions, deliveryoptions }) => {
	...
	const companyrequired = ['company_name', 'pib', 'maticni_broj', 'shipping_company_name'];
	...
	const formSubmitHandler = () => {
		...
		for (const key in formData) {
			...
			if (
				(formData.type === 'company' && companyrequired.includes(key) && (item === '' || item == null)) ||
		...
		if (err.length > 0) {
			...
			const ret = {
				...
				maticni_broj_shipping: formData.type === 'company' ? formData.maticni_broj : null,
				first_name_shipping: secondAddress ? formData.shipping_first_name : formData.first_name,
				last_name_shipping: secondAddress ? formData.shipping_last_name : formData.last_name,
				phone_shipping: secondAddress ? formData.shipping_phone : formData.phone,
				email_shipping: secondAddress ? formData.shipping_email : formData.email,
				address_shipping: secondAddress ? formData.shipping_address : formData.address,
				object_number_shipping: secondAddress ? formData.shipping_object_number : formData.object_number,
				floor_shipping: '',
				apartment_number_shipping: '',
				id_town_shipping: null,
				town_name_shipping: secondAddress ? formData.shipping_town : formData.town,
				zip_code_shipping: secondAddress ? formData.shipping_zip_code : formData.zip_code,
				...
				company_name_billing: formData.type === 'company' ? formData.company_name : null,
				pib_billing: formData.type === 'company' ? formData.pib : null,
				maticni_broj_billing: formData.type === 'company' ? formData.maticni_broj : null,
	...
	return (
		<>
			<Seo
				title="Vaša korpa"
				description="Vaša korpa"
				ogtitle="Vaša korpa"
				ogdescription="Vaša korpa"
				ogurl={`${process.env.BASE_URL}korpa`}
			/>
					...
					{cartItems.length > 0 ? (
						<>
								...
								<div className={classes['user-info']}>
									...
									{formData.type === 'company' && (
										<>
											<div className={`${classes['input-info-container']} row`}>
												<div className={`${classes['input-info']} col-lg-12 col-12`}>
													<label className={classes['input-label']} htmlFor="company_name">
											...
											<div className={`${classes['input-info-container']} row`}>
												...
												<div className={`${classes['input-info-right']} col-lg-6 col-12`}>
													<label className={classes['input-label']} htmlFor="maticni_broj">
									...
									<div className={`${classes['input-info-container']} row`}>
										...
										<div className={`${classes['input-info-right']} col-lg-6 col-12`}>
											...
											<div className="row">
												<div className="col-6">
													<label className={classes['input-label']} htmlFor="address">
												...
												<div className="col-6">
													<label className={classes['input-label']} htmlFor="object_number">
											...
											<div className="row">
												<div className="col-6">
													<label className={classes['input-label']} htmlFor="zip_code">
									...
									<div className={`${classes['additional-user-info']} row`}>
										<div className={`${classes['user-info-adress']} col-lg-6 col-12`}>
											...
											<p className={classes['user-info-adress-p']}>
												Zahtevajte drugu adresu za dostavu u odnosu na adresu računa.
											</p>
									...
									{secondAddress && (
										<>
											<div className={`${classes['input-info-container']} row`}>
												<div className={`${classes['input-info-left']} col-lg-6 col-12`}>
													<label className={classes['input-label']} htmlFor="shipping_name">
														Ime: <span className="mandatory">*</span>
													...
													<label className={classes['input-label']} htmlFor="shipping_email">
														E-mail: <span className="mandatory">*</span>
												...
												<div className={`${classes['input-info-right']} col-lg-6 col-12`}>
													<label className={classes['input-label']} htmlFor="shipping_phone">
														Telefon: <span className="mandatory">*</span>
											...
											<div className={`${classes['additional-user-info']} row`}>
												<div className={`${classes['input-info']} col-lg-12 col-12`}>
													<label className={classes['input-label']} htmlFor="shipping_note">
														Napomena:
													</label>
													<div className={classes['input-info-padding']}>
														<textarea
									...
									<div className={classes['value-container']}>
										...
										<span className={classes['value-span']}>
											...
											<span className={classes['value-span-right']}>
												{currencyFormat(cartCost?.totals?.with_vat ?? 0, cartCost?.currency)}
											</span>
										</span>
										<span className={classes['value-span']}>
										...
										<span className={classes['value-span']}>
											Iznos PDV-a:
											<span className={classes['value-span-right']}>
												{currencyFormat(cartCost?.totals?.vat ?? 0, cartCost?.currency)}
											</span>
										</span>
										<span className={classes['value-span']}>
											Ukupna vrednost korpe nakon popusta:
											<span className={classes['value-span-right-sum']}>
												{currencyFormat(cartCost?.totals?.total ?? 0, cartCost?.currency)}
											</span>
										</span>
									</div>
								</div>
							...
							<div className={classes.agree}>
								...
								{errors.includes('agreed') && <span className={classes.errorMsg}>{errorChecked}</span>}
							</div>
...
export const getStaticProps = async () => {
	...
	return {
		props: {
			paymentoptions: await api.get('checkout/payment-options').then((response) => response?.payload),
			deliveryoptions: await api.get('checkout/delivery-options').then((response) => response?.payload),
		},
	};
};
